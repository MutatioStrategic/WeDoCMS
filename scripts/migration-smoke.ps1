...
$foreignKeys = npx wrangler d1 execute veld-archive --local --command "PRAGMA foreign_key_check" --json | ConvertFrom-Json
$foreignKeyRows = @($foreignKeys | ForEach-Object { $_.results } | Where-Object { $_ })
if ($foreignKeyRows.Count -gt 0) { throw "Foreign-key check returned invalid rows: $($foreignKeyRows | ConvertTo-Json -Compress)" }
