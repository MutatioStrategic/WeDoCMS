...
- Photo enrichment pipeline: image upload → queued Workers AI visual metadata/OCR → seller/editor correction and approval. General buyer search is deterministic and reads approved title/description metadata. Opt-in AI search interprets a natural-language brief against vectors built from the current approved metadata, then rechecks the published D1 revision before returning a result; source images are never re-analysed at query time.
