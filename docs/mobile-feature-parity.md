...
| Upload and submission status | Native | upload-session and asset routes | Uses the platform image picker and the existing private upload workflow. |
...
| Account, alerts, data rights, licences | Native core path | `/api/account/*`, `/api/notifications`, `/api/licences/history` | Includes identity portal, preferences, export, deletion, alerts, and licence history. Licensed file delivery remains desktop-only. |
