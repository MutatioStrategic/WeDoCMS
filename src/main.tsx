...
const workerApiOrigin = ((import.meta.env.VITE_WORKER_API_URL as string | undefined)?.trim()
  || (typeof window !== "undefined" && window.location.hostname === "veld-archive.pages.dev" ? "https://veld-archive-api-production.blewisorlando.workers.dev" : "")).replace(/\/$/, "");
const apiUrl = (path: string): string => `${workerApiOrigin}${path}`;
...
function App({ auth0, supabase }: { auth0?: Auth0Bridge; supabase?: SupabaseClient }) {
  ...
  const api = useCallback((path: string, init: RequestInit = {}) => fetch(apiUrl(path), { ...init, credentials: "include", headers: { ...(init.body ? { "Content-Type": "application/json" } : {}), ...(csrfToken ? { "X-CSRF-Token": csrfToken } : {}), ...(init.headers ?? {}) } }), [csrfToken]);

  useEffect(() => {
    void fetch(apiUrl("/api/auth/session"), { credentials: "include" }).then(async (response) => {
  ...
  useEffect(() => {
    ...
    void auth0.getAccessTokenSilently({ authorizationParams: { scope: auth0Scopes, ...(auth0Audience ? { audience: auth0Audience } : {}) } })
      .then((token) => fetch(apiUrl("/api/auth/exchange"), {
        method: "POST",
        credentials: "include",
  ...
  const exchangeSupabaseSession = useCallback(async (session: SupabaseSession | null): Promise<void> => {
    ...
    try {
      const response = await fetch(apiUrl("/api/auth/exchange"), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` }, body: "{}" });
      if (!response.ok) throw new Error("Identity exchange failed");
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/assets?${params}`), { signal: controller.signal, credentials: "include" })
  ...
  async function devSignIn(): Promise<void> {
    const response = await fetch(apiUrl("/api/auth/dev-login"), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ role: devRole }) });
  ...
  function trackEvent(payload: Record<string, unknown>) {
    ...
    void fetch(apiUrl("/api/analytics/events"), { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...payload, consent: true }) }).catch(() => undefined);
  }
...
function AnalyticsDashboard({ role, onOpenAccount }: { role: "contributor" | "buyer"; onOpenAccount?: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl(`/api/analytics/${role}`), { signal: controller.signal, credentials: "include" })
...
function CampaignWorkspace({ api, onNotice, onOpen }: { api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; onOpen: (asset: Asset) => void }) {
  ...
  return <main className="campaign-page">
    ...
    {activeCampaign && <section className="campaign-editor"><div className="campaign-editor-heading"><div><span className="section-kicker">ACTIVE WORKSPACE</span><h2>{activeCampaign.name}</h2><p>{activeCampaign.brief}</p></div><button className="outline-button" type="button" onClick={() => void downloadManifest()}>Export campaign pack ↗</button></div><div className="brief-pills"><span>{activeCampaign.briefFields.usageRights} use</span>{activeCampaign.briefFields.platforms.map((platform) => <span key={platform}>{campaignPlatformLabel(platform)}</span>)}{activeCampaign.briefFields.tone.map((value) => <span key={value}>{value}</span>)}</div>{selectedAsset && <CampaignFormatPlanner asset={selectedAsset} onApprove={() => void changeStage(selectedRecommendation!, "approved")} onNotice={onNotice} />}
...
function CreatorMarketplace({ onOpen }: { onOpen: (asset: Asset) => void }) {
  ...
  async function openCreator(slug: string) { const response = await fetch(apiUrl(`/api/creators/${slug}`)); if (!response.ok) return; setSelected(await response.json() as { profile: CreatorProfile; assets: Asset[]; collections: PortfolioCollection[] }); window.history.replaceState(null, "", `/creators/${slug}`); }
...
function GovernanceDetail({ asset, licenceType, setLicenceType, validation, onAction, onCheckout, buyerTermsAccepted, setBuyerTermsAccepted }: { asset: Asset; licenceType: LicenceType; setLicenceType: (value: LicenceType) => void; validation: ReturnType<typeof archiveDomain.evaluateLicenceRequest>; onAction: (name: "run_ai_tagging" | "save_correction" | "approve", updates?: Partial<Asset>) => void; onCheckout: () => void; buyerTermsAccepted: boolean; setBuyerTermsAccepted: (value: boolean) => void }) {
  ...
  return <article className="governance-detail">
    ...
    <div className="governance-preview"><AssetPreview asset={asset} className="governance-preview-media" /><small>SOURCE · {asset.sourceFileName ?? "source file pending"}</small><b>{asset.authenticityConfidence ? `${Math.round(asset.authenticityConfidence * 100)}%` : "—"}<em>AI confidence</em></b></div>
...
function ReviewWorkspace({ items, api, onNotice, onReload }: { items: Asset[]; api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; onReload: () => Promise<void> }) {
  ...
  const [selectedId, setSelectedId] = useState("");
  const selected = items.find((asset) => asset.id === selectedId) ?? items[0] ?? null;
  useEffect(() => { if (items.length && !items.some((asset) => asset.id === selectedId)) setSelectedId(items[0].id); }, [items, selectedId]);
  ...
  return <main className="workspace-page"><div className="workspace-intro"><span className="section-kicker">EDITORIAL GOVERNANCE / ADMIN SESSION</span><h1>Review what is <em>real.</em></h1><p>One selected record, clear evidence, one next action. Approve only after place, context, rights, consent, and seller identity are understood.</p></div><section className="review-queue"><div className="card-heading"><span className="section-kicker">PENDING TENDERS</span><span>{tenders.length} seller submissions</span></div>{tenders.length ? tenders.map((tender) => <article className="review-item" key={String(tender.id)}><div className="review-copy"><div className="card-heading"><span className="section-kicker">{String(tender.id).slice(0, 8)} · {String(tender.created_at ?? "")}</span><span className="status-pill warm">{String(tender.status)}</span></div><h2>{String(tender.display_name)}</h2><p>{String(tender.email)} · contract {String(tender.contract_version)} · hash {String(tender.contract_hash).slice(0, 16)}…</p><div className="review-evidence"><span>KYC {String(tender.verification_status ?? "missing")}</span><span>Wallet {String(tender.wallet_provider ?? "missing")} / {String(tender.wallet_status ?? "missing")}</span><span>Risk {String(tender.risk_level ?? "unknown")}</span></div><div className="review-actions">{tender.wallet_id && tender.wallet_status !== "verified" && <button className="outline-button" onClick={() => verifyWallet(tender)}>Verify wallet</button>}<button className="dark-button" onClick={() => decideTender(tender, "approved")}>Accept tender</button><button className="ghost-button" onClick={() => decideTender(tender, "corrections_requested")}>Request corrections</button><button className="ghost-button danger-button" onClick={() => decideTender(tender, "rejected")}>Reject</button></div></div></article>) : <div className="empty-state">No seller tenders are waiting for review.</div>}</section><section className="review-panel"><div className="review-panel-heading"><div><span className="section-kicker">PHOTO REVIEW QUEUE</span><h2>{items.length ? `${items.length} records need a decision` : "No records need a decision"}</h2></div><span className="status-pill cool">100-photo library stays searchable</span></div>{items.length ? <div className="review-panel-layout"><div className="review-record-list" aria-label="Photo review queue">{items.map((asset) => <button type="button" className={`review-record ${asset.id === selected?.id ? "selected" : ""}`} key={asset.id} onClick={() => setSelectedId(asset.id)}><AssetPreview asset={asset} className="review-record-thumb" /><span><strong>{asset.title}</strong><small>{assetPlace(asset)}</small><em>{asset.rightsStatus} · {asset.humanVerified ? "verified" : "needs review"}</em></span></button>)}</div>{selected && <article className="review-detail"><div className="review-detail-heading"><div><span className="section-kicker">SELECTED PHOTO / {selected.id.slice(0, 12)}</span><h3>{selected.title}</h3><p>{assetPlace(selected)} · by {selected.contributor}</p></div><span className="status-pill warm">Needs review</span></div><AssetPreview asset={selected} className="review-detail-photo" /><div className="review-detail-grid"><div><span className="review-detail-label">Evidence & metadata</span><div className="review-detail-facts"><span><b>Rights</b>{selected.rightsStatus}</span><span><b>Model release</b>{selected.modelReleaseStatus}</span><span><b>Property release</b>{selected.propertyReleaseStatus}</span><span><b>AI confidence</b>{archiveDomain.percent(selected.authenticityConfidence)}%</span></div></div><div><span className="review-detail-label">Context tags</span><div className="review-tags">{[...selected.subjectTags, ...selected.culturalTags].slice(0, 8).map((tag) => <span key={tag}>{tag}</span>)}</div></div></div><p className="review-detail-note">{selected.caption || selected.description || "Add a precise caption and evidence note before publishing."}</p><div className="review-actions review-detail-actions"><button className="dark-button" onClick={() => decide(selected, "approved")}>Approve & publish</button><button className="outline-button" onClick={() => decide(selected, "needs_changes")}>Request changes</button><button className="ghost-button danger-button" onClick={() => decide(selected, "rejected")}>Reject</button></div></article>}</div> : <div className="review-empty-next"><strong>Queue clear.</strong><span>New contributor submissions will appear here after ingestion and media processing.</span></div>}</section></main>;
}
