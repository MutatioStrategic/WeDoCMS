...
function CampaignWorkspace({ api, onNotice, onOpen, role }: { api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; onOpen: (asset: Asset) => void; role?: string }) {
  ...
  const loadCampaign = useCallback(async (id: string) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error();
      const data = await response.json() as { campaign: CampaignSummary; assets?: CmsCampaignAsset[]; recommendations: CampaignRecommendationRow[]; editVersions?: CmsDetail["editVersions"]; derivatives?: CmsDerivative[]; bundles?: CmsBundle[]; licenceMetadata?: CmsLicenceMetadata[]; blockers?: CmsBlocker[]; buyerTermsAccepted?: boolean };
      ...
      setBuyerTermsAccepted(Boolean(data.buyerTermsAccepted));
  ...
  async function changeStage(item: CampaignRecommendationRow, stage: CampaignStage) {
    ...
    if (stage === "approved" && isBuyer && !buyerTermsAccepted) { onNotice("View and accept the campaign terms before approving a source."); return; }
    try {
  ...
  return <main className="campaign-page">
    ...
    {activeCampaign && <section className="campaign-editor"><div className="campaign-editor-heading"><div><span className="section-kicker">ACTIVE WORKSPACE</span><h2>{activeCampaign.name}</h2><p>{activeCampaign.brief}</p></div><button className="outline-button" type="button" onClick={() => void downloadManifest()}>Export campaign pack ↗</button></div><div className="brief-pills"><span>{activeCampaign.briefFields.usageRights} use</span>{activeCampaign.briefFields.platforms.map((platform) => <span key={platform}>{campaignPlatformLabel(platform)}</span>)}{activeCampaign.briefFields.tone.map((value) => <span key={value}>{value}</span>)}</div>
      ...
      {activeCampaign && isBuyer && <CampaignTermsGate campaignId={activeCampaign.id} accepted={buyerTermsAccepted} api={api} onNotice={onNotice} onAccepted={() => setBuyerTermsAccepted(true)} />}
...
function AuthenticatedDerivativeEditor({ asset, versions, licenceId, campaignId, api, onNotice, onSaved }: { asset: CmsCampaignAsset; versions: CmsDetail["editVersions"]; licenceId: string | null; campaignId: string; api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    setRecipe(defaultEditRecipe()); setState("idle"); setError("");
  ...
  const render = useCallback(() => {
    ...
    const preset = cropPresets[recipe.preset]; canvas.width = preset.width; canvas.height = preset.height;
    const context = canvas.getContext("2d"); if (!context) return;
    const crop = fitCrop(image.naturalWidth, image.naturalHeight, preset.ratio);
    context.fillStyle = "#1d211d"; context.fillRect(0, 0, canvas.width, canvas.height);
    context.filter = `brightness(${recipe.brightness}%) contrast(${recipe.contrast}%) saturate(${recipe.saturation}%)`;
    context.drawImage(image, crop.x, crop.y, crop.width, crop.height, 0, 0, canvas.width, canvas.height); context.filter = "none";
  }, [image, recipe]);
  useEffect(() => { render(); }, [render]);
  async function saveVersionAndExport(): Promise<void> {
    ...
    try {
      ...
      const exportCanvas = document.createElement("canvas"); exportCanvas.width = cropPresets[recipe.preset].width; exportCanvas.height = cropPresets[recipe.preset].height;
      const exportContext = exportCanvas.getContext("2d"); if (!exportContext) throw new Error("The browser could not prepare the derivative.");
      render(); if (!canvasRef.current) throw new Error("The browser preview is unavailable."); exportContext.drawImage(canvasRef.current, 0, 0, exportCanvas.width, exportCanvas.height);
      const blob = await new Promise<Blob | null>((resolve) => exportCanvas.toBlob(resolve, "image/webp", .9)); if (!blob) throw new Error("The browser could not encode the derivative.");
      ...
      onNotice(`Version ${versionBody.versionNumber ?? ""} exported. The original remains unchanged.`); onSaved();
    } catch (failure) { setState("failed"); setError(failure instanceof Error ? failure.message : "The edit could not be saved or exported."); onNotice("The edit could not be saved or exported. The source asset was not changed."); }
...
function CampaignTermsGate({ campaignId, accepted, api, onNotice, onAccepted }: { campaignId: string; accepted: boolean; api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; onAccepted: () => void }) {
  const [viewed, setViewed] = useState(accepted);
  const [saving, setSaving] = useState(false);
  useEffect(() => { if (accepted) setViewed(true); }, [accepted]);
  async function accept(): Promise<void> {
    if (!viewed || saving || accepted) return;
    setSaving(true);
    try {
      const response = await api(`/api/campaigns/${encodeURIComponent(campaignId)}/terms/accept`, { method: "POST", body: JSON.stringify({ viewed: true, accepted: true, buyerAgreementVersion: buyerAgreement.version, paymentAgreementVersion: paymentDisclosure.version }) });
      const body = await response.json().catch(() => ({})) as { error?: string };
      if (!response.ok) throw new Error(body.error ?? "The campaign terms could not be accepted.");
      onAccepted(); onNotice("Campaign terms accepted. Licensed sources can now be approved and delivered.");
    } catch (failure) { onNotice(failure instanceof Error ? failure.message : "The campaign terms could not be accepted."); }
    finally { setSaving(false); }
  }
  return <section className={`campaign-terms-gate ${accepted ? "accepted" : ""}`} aria-labelledby="campaign-terms-heading-main"><div className="card-heading"><div><span className="section-kicker">BUYER APPROVAL GATE</span><h3 id="campaign-terms-heading-main">Read the terms before approving delivery.</h3></div><span className={`status-pill ${accepted ? "cool" : "warm"}`}>{accepted ? "Accepted" : "Required"}</span></div><p>Approval confirms use only within the displayed licence, territory, duration, attribution, and restrictions. Copyright stays with the contributor.</p><details onToggle={(event) => { if (event.currentTarget.open) setViewed(true); }}><summary>View buyer licence and payment terms Â· {buyerAgreement.version} / {paymentDisclosure.version}</summary><div className="campaign-terms-copy"><h4>{buyerAgreement.title}</h4>{buyerAgreement.sections.map((section) => <section key={section.heading}><strong>{section.heading}</strong><p>{section.body}</p></section>)}<h4>{paymentDisclosure.title}</h4>{paymentDisclosure.sections.map((section) => <section key={section.heading}><strong>{section.heading}</strong><p>{section.body}</p></section>)}</div></details><label className="checkbox-row campaign-terms-check"><input type="checkbox" checked={accepted} disabled={!viewed || saving} onChange={() => void accept()} /> I have read and accept these current terms for this campaign.</label>{!viewed && <small className="field-help">Open the terms first. Approval controls remain locked until they are viewed and accepted.</small>}</section>;
}
...
function CampaignDeliveryPanel({ campaign, selectedAssetId, assets, editVersions, derivatives, bundles, licenceMetadata, blockers, role, api, onNotice, onRefresh }: { campaign: CampaignSummary; selectedAssetId: string; assets: CmsCampaignAsset[]; editVersions: CmsDetail["editVersions"]; derivatives: CmsDerivative[]; bundles: CmsBundle[]; licenceMetadata: CmsLicenceMetadata[]; blockers: CmsBlocker[]; role?: string; api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; onRefresh: () => void }) {
  const [bundleType, setBundleType] = useState("social_media"); const [bundleBusy, setBundleBusy] = useState("");
  ...
  async function requestBundle(): Promise<void> { setBundleBusy("request"); try { const response = await api(`/api/campaigns/${encodeURIComponent(campaign.id)}/bundles`, { method: "POST", body: JSON.stringify({ bundleType }) }); const body = await response.json().catch(() => ({})) as { error?: string; blockers?: Array<{ message?: string }> }; if (!response.ok) throw new Error(body.blockers?.[0]?.message ?? body.error ?? "The bundle is not ready."); onNotice("Bundle requested. A reviewer must approve the auditable ZIP before download."); onRefresh(); } catch (failure) { onNotice(failure instanceof Error ? failure.message : "The bundle request failed."); } finally { setBundleBusy(""); } }
  async function approveBundle(bundle: CmsBundle): Promise<void> { setBundleBusy(bundle.id); try { const response = await api(`/api/campaigns/${encodeURIComponent(campaign.id)}/bundles/${encodeURIComponent(bundle.id)}/approve`, { method: "POST", body: "{}" }); const body = await response.json().catch(() => ({})) as { error?: string }; if (!response.ok) throw new Error(body.error ?? "Bundle approval failed."); onNotice("Bundle approved and built as an auditable streamed ZIP."); onRefresh(); } catch (failure) { onNotice(failure instanceof Error ? failure.message : "Bundle approval failed."); } finally { setBundleBusy(""); } }
...
type MarketplaceAgreementDocument = {
  type: "seller" | "buyer" | "payment";
  version: string;
  title: string;
...
function AssetModal({ asset, api, autoOpenPurchase = false, onClose, onNotice: notify, onRequireSignIn, authenticated, lightboxes, onCreateLightbox, onSaveToLightbox, onDownload }: {
  ...
  const [agreementState, setAgreementState] = useState<"idle" | "loading" | "ready" | "error">("idle");
  ...
  const buyerAgreement = agreements.find((document) => document.type === "buyer");
  const paymentDisclosure = agreements.find((document) => document.type === "payment");
  ...
  const loadAgreements = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const body = await response.json().catch(() => ({})) as { documents?: MarketplaceAgreementDocument[]; error?: string };
      if (!response.ok || !body.documents?.some((document) => document.type === "buyer") || !body.documents.some((document) => document.type === "payment")) {
        throw new Error(body.error ?? "The current buyer and payment terms could not be loaded.");
      }
      setAgreements(body.documents);
      setAgreementState("ready");
    } catch (error) {
      setAgreements([]);
      setAgreementState("error");
      setPurchaseError(error instanceof Error ? error.message : "The current buyer and payment terms could not be loaded.");
    }
  }, [api]);
  ...
  useEffect(() => {
    ...
    void loadAgreements();
  }, [authenticated, loadAgreements, model, purchaseOpen]);
  ...
  async function purchaseLicence(): Promise<void> {
    if (!authenticated) { onRequireSignIn(asset.id); return; }
    if (!buyerAgreement || !paymentDisclosure) {
      setPurchaseError("The current buyer and payment terms could not be loaded. Try again before continuing.");
      return;
    }
    ...
    <div className="asset-modal" role="dialog" aria-modal="true" aria-labelledby="asset-title" onClick={(event) => event.stopPropagation()}>
      ...
      <div className="modal-copy">
        ...
        {purchaseOpen && <section className="asset-purchase-panel" aria-labelledby="asset-purchase-title">
          ...
          {agreementState === "loading" && <p className="purchase-feedback" role="status" aria-live="polite">Loading the current buyer and payment terms…</p>}
          {agreementState === "error" && <p className="purchase-feedback error" role="alert">{purchaseError || "The current buyer and payment terms could not be loaded."} <button type="button" className="text-button" onClick={() => void loadAgreements()}>Try again</button></p>}
          {buyerAgreement && paymentDisclosure ? <details className="purchase-terms" onToggle={(event) => { if (event.currentTarget.open) setTermsViewed(true); }}><summary>Read {buyerAgreement.title} and {paymentDisclosure.title}</summary><div><h4>{buyerAgreement.title} · {buyerAgreement.version}</h4>{buyerAgreement.sections.map((section) => <p key={section.heading}><strong>{section.heading}</strong> {section.body}</p>)}<h4>{paymentDisclosure.title} · {paymentDisclosure.version}</h4>{paymentDisclosure.sections.map((section) => <p key={section.heading}><strong>{section.heading}</strong> {section.body}</p>)}</div></details> : agreementState !== "loading" && <p className="purchase-feedback" role="status">Payment cannot continue until the current terms are available.</p>}
          <label className="purchase-terms-check"><input type="checkbox" checked={termsAccepted} disabled={!termsViewed || purchaseBusy || !buyerAgreement || !paymentDisclosure} onChange={(event) => setTermsAccepted(event.target.checked)} /><span>I have read and accept the displayed buyer licence and payment terms for this selected use.</span></label>
          {purchaseError && validationState !== "error" && <p className="purchase-feedback error" role="alert">{purchaseError}</p>}
          <div className="purchase-actions"><button type="button" className="outline-button" onClick={() => void validateLicence()} disabled={validationState === "loading" || purchaseBusy}>Check licence again</button><button type="button" className="dark-button" onClick={() => void purchaseLicence()} disabled={purchaseBusy || validationState === "loading" || agreementState !== "ready" || !validation?.allowed || !termsAccepted}>{purchaseBusy ? "Preparing secure checkout…" : demoMode ? "Simulate purchase (no charge)" : "Purchase licence"} ↗</button></div>
        </section>}
