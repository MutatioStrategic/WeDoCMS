...
import { archiveDomain, INTRODUCTORY_FREE_CREDIT_COST, INTRODUCTORY_FREE_CREDITS, INTRODUCTORY_FREE_DOWNLOAD_LIMIT, MEDIA_MEMBERSHIP_CREDITS, MEDIA_MEMBERSHIP_DURATION_DAYS, mediaCreditReferenceAmountCents, mediaMembershipDurationLabel, type AccountLifecycle, type Asset, type BuyerAnalytics, type CommunityOverview, type ContributorAnalytics, type ContributorPerformance, type CreatorProfile, type DiscoveryResponse, type LicenceProduct, type LicenceType, type MonetizationModel, type PortfolioCollection, type SavedSearch, type SearchResponse, type TakedownReason, type UserLightbox, type WorkflowStage } from "./shared";
...
const liveDemoAccounts = [
  { role: "buyer", label: "Buyer", email: "buyer@demo.veld", detail: "Licence & download approved media." },
  { role: "contributor", label: "Seller", email: "seller@demo.veld", detail: "Upload, correct metadata, sell." },
  { role: "editor", label: "Editor / Admin", email: "editor@demo.veld", detail: "Review evidence & govern." },
] as const;

function LiveDemoAccounts({ onSelect }: { onSelect: (account: typeof liveDemoAccounts[number]) => void }) {
  return <section className="live-demo-accounts" aria-labelledby="live-demo-accounts-title">
    <span className="section-kicker" id="live-demo-accounts-title">DEMO ACCOUNTS</span>
    {liveDemoAccounts.map((account) => <button type="button" className="live-demo-account" key={account.role} onClick={() => onSelect(account)}>
      <span><strong>{account.label}</strong><small>{account.detail}</small></span><code>{account.email}</code>
    </button>)}
  </section>;
}
...
function App({ auth0, supabase, authRedirectUrl = defaultAuthRedirectUrl, authLoading = false, authUnavailable = false, onRetryAuth }: AppProps) {
  ...
  async function submitSupabaseAuth(event: React.FormEvent): Promise<void> {
    ...
    try {
      ...
      if (result.data.session) {
        ...
        setNotice("Account created. Confirm your email, then sign in to claim 300 starter credits for 3 approved photos.");
      }
    } catch (error) {
  ...
  return <div className={`app-shell better-shell ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}>
    ...
    {demoMode && !sessionUser && <LiveDemoAccounts onSelect={(account) => void devSignIn(account.role)} />}
    {supabase && supabaseAuthOpen && !sessionUser && supabaseAuthMode === "signin" && supabaseAuthMethod === "email" && <LiveDemoAccounts onSelect={(account) => { setSupabaseEmail(account.email); setSupabasePassword(""); setSupabaseAuthMessage(null); }} />}
...
type BuyerFinanceSnapshot = {
  ...
  credits: { creditPackCredits?: number; creditPackDurationDays?: number; balanceCredits: number; introductoryFreeCredits?: number; introductoryFreeCreditCost?: number; introductoryFreeDownloadLimit?: number; transactions: Array<Record<string, unknown>>; pendingPurchases: Array<Record<string, unknown>>; showCurrencyReference?: boolean; referenceAmountCents?: number };
};
...
function BuyerFinancePanel({ api, onNotice, role, account }: { api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; role?: string; account: Pick<SessionUser, "displayName" | "id" | "email"> | null }) {
  ...
  const introductoryCredits = snapshot?.credits.introductoryFreeCredits ?? INTRODUCTORY_FREE_CREDITS;
  const introductoryCreditCost = snapshot?.credits.introductoryFreeCreditCost ?? INTRODUCTORY_FREE_CREDIT_COST;
  const introductoryPhotoLimit = snapshot?.credits.introductoryFreeDownloadLimit ?? INTRODUCTORY_FREE_DOWNLOAD_LIMIT;
...
function BuyerSubscriptionPanel({ api, onNotice, account }: { api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void; account: Pick<SessionUser, "displayName" | "id" | "email"> | null }) {
  const [data, setData] = useState<{ configured: boolean; hasAccess: boolean; sourceOfTruth: string; plans: Array<{ id: "monthly" | "annual"; amountCents: number; currency: string; interval: string }>; plan: { amountCents: number; currency: string; interval: string } | null; subscription: Record<string, unknown> | null; payments: Array<Record<string, unknown>>; free: { limit: number; used: number; remaining: number; freeCreditCost?: number; freeCreditsRemaining?: number }; credits: { balanceCredits: number } } | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [busy, setBusy] = useState(false);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const free = await freeResponse.json() as { limit: number; used: number; remaining: number; freeCreditCost?: number; freeCreditsRemaining?: number };
