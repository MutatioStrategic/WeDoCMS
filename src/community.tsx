...
import type { CommunityOverview, TakedownReason } from "./shared";
...
export function CommunityWorkspace({ api, onNotice }: { api: (path: string, init?: RequestInit) => Promise<Response>; onNotice: (notice: string) => void }) {
  ...
  useEffect(() => { api("/api/community/overview").then(async (response) => { if (response.ok) setData(await response.json() as CommunityOverview); }).catch(() => undefined); }, [api]);
