...
export const sellerAgreement: MarketplaceAgreement = {
  ...
  sections: [
    ...
    { heading: "5. Credit access, Paystack split and your share", body: "Buyer media access is presented as a credit membership. The standard one-year access term costs 100 credits; longer terms use the published credit multiple. If a currency reference is shown, it is display-only. A payment provider may still collect a local-currency reference amount to complete settlement. Subject to Paystack approval and the applicable provider terms, WeDoCMS uses a Paystack marketplace split so your verified Paystack subaccount is allocated the agreed percentage (for example 60%) at the transaction, while the balance is allocated to WeDoCMS for its platform commission. Payment-provider fees, taxes, refunds, chargebacks, reserves and settlement timing may affect the amount actually available. WeDoCMS does not promise a payout until Paystack settles it." },
...
export const buyerAgreement: MarketplaceAgreement = {
  ...
  sections: [
    ...
    { heading: "2. Licence you receive", body: "After successful payment, you receive only the usage rights described by the listing's selected licence, territory, duration and product restrictions. Standard media access is sold as a credit membership: 100 credits unlock 12 months of access, and the selected duration shows the required credit multiple. You do not receive ownership, an unrestricted resale right, a right to sublicense, or a right to remove attribution unless the selected licence expressly grants it." },
    { heading: "3. Credit price and payment destination", body: "The buyer price is the credit amount shown at checkout. Credits unlock downloads or streams and are valid for the stated access duration from purchase. If a currency line is shown, it is labelled display-only and is only a reference for the payment provider; it is not a second offer or a negotiated price. Paystack may process the local-currency reference amount and, where enabled, split settlement between the Seller's verified Paystack account and WeDoCMS's platform account. Provider fees, tax treatment, refunds, chargebacks and settlement timing are handled under the applicable provider and platform rules." },
...
export const paymentDisclosure: MarketplaceAgreement = {
  type: "payment",
  version: "payment-split-v2",
  title: "WeDoCMS Payment and Paystack Split Disclosure",
  audience: "both",
  effectiveDate,
  draft: false,
  sections: [
    { heading: "Buyer money flow", body: "The buyer selects the displayed credit membership and opens the Paystack checkout initiated by WeDoCMS. Any local-currency amount collected by the provider is a display-only settlement reference; the buyer product remains the stated credit amount. Paystack processes the payment. A successful transaction is recorded against the licence only after a signed provider webhook is reconciled." },
