...
import { decryptSecret, encryptSecret, type EncryptedSecret } from "./secret-crypto";
...
export async function encryptZohoSecret(secret: string, encryptionKey: string): Promise<EncryptedSecret> {
  return encryptSecret(secret, encryptionKey, "ZOHO_TOKEN_ENCRYPTION_KEY");
}

export async function decryptZohoSecret(ciphertext: string, iv: string, encryptionKey: string): Promise<string> {
  return decryptSecret(ciphertext, iv, encryptionKey, "ZOHO_TOKEN_ENCRYPTION_KEY");
}
