...
export class IntegrationContainer {
  ...
  constructor(environment: IntegrationEnvironment) {
    ...
    if (diditApiKey && environment.DIDIT_KYC_WORKFLOW_ID && environment.DIDIT_KYB_WORKFLOW_ID) this.didit = new DiditVerificationAdapter({ apiKey: diditApiKey, kycWorkflowId: environment.DIDIT_KYC_WORKFLOW_ID, kybWorkflowId: environment.DIDIT_KYB_WORKFLOW_ID, endpoint: environment.DIDIT_API_URL ?? environment.DIDIT_URL });
